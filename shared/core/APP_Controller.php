...
class APP_Controller extends CI_Controller {
     ...
     * @return array
     */
    public function read_csv_file($file_path, $separator = "\t", $enclosure = '"', $encode_from = 'UTF-16LE')
    {
        $empty = "";
        ...
        //Get content text in file
        $csv = stream_get_contents($file);

        //remove chr(255) and  chr(254)
        $csv = str_replace(chr(255) . chr(254), $empty, $csv);

        //encoding to UTF8
        $decoded_csv = mb_convert_encoding($csv, 'UTF-8', $encode_from);
        if(!empty($enclosure)) {
            $decoded_csv = str_replace($enclosure, $empty, $decoded_csv);
        }

        $lines = explode(PHP_EOL, $decoded_csv);
        $array = array();

        //to array
        foreach ($lines as $key => $line) {
            if(!(empty(trim($line)) && !isset($lines[$key + 1]))) {
                $array[] = str_getcsv($line, $separator);
            }
        }
