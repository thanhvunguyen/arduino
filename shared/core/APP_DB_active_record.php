...
class APP_DB_active_record extends CI_DB_query_builder {
     ...
     * @throws APP_DB_exception
     */
    public function insert_batch($table = '', $set = NULL, $escape = NULL, $options = array())
    {
        if ($set !== NULL)
        {
        ...
        {
            $this->query($this->_insert_batch($this->protect_identifiers($table, TRUE, $escape, FALSE), $this->qb_keys, array_slice($this->qb_set, $i, 100), $options));
     ...
     * @return string
     */
    protected function _insert_batch($table, $keys, $values, $options = [])
    {
        $sql = 'INSERT INTO ';
        if (array_key_exists('mode', $options) && $options['mode'] == 'ignore') {
            $sql = 'INSERT IGNORE INTO ';
        } else if (array_key_exists('mode', $options) && $options['mode'] == 'replace'){
            $sql = 'REPLACE INTO ';
        }

        return $sql.$table.' ('.implode(', ', $keys).') VALUES '.implode(', ', $values);
    }
