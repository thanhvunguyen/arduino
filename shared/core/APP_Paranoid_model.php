...
class APP_Paranoid_model extends APP_Model
{
    ...
    /**
     *  Select all (content deleted)
     *
     * @param array $options
     * @return array
     */
    public function real_all($options = array())
    {
        return parent::all($options);
    }
     ...
     * @return array
     */
    public function real_first($options = array())
    {
        $this->limit(1);
        $result = $this->real_all($options);
        if (FALSE === $result) {
            return FALSE;
        }
     ...
     * @return false|int 削除件数
     */
    public function real_destroy_all_without_fk($options = array())
    {

        return parent::destroy_all_without_fk($options);
    }
