...
class APP_Api_validation extends APP_Form_validation {
    ...
    public $_schemas = [
            'example_sentence' =>
                array(
                    'id' => 'ID',
                    'japanese_sentence' => '日本語文',
                    'hiragana_sentence' => 'ひらがな',
                    'alphabet_sentence' =>'アルファベット',
                ),
            ...
            'end_user_list' =>
                array(
                    ...
                    'email' => 'メールアドレス',
                    'login_id' =>'ユーザーID',
                    'date_started_using' => '利用開始日',
                    'number_time_practice' =>'練習回数',
                    'highest_score' => '最高スコア',
                    'average_score' => '平均スコア',
                    'growth_rate' => 'スコア成長率'
                )
    ...
    public function error_json($extra = array(), $options = array(), $render = TRUE)
    {
        ...
        if($this->_admin_required && !$this->base->operator()->is_administrator()) {
            return $this->base->false_json(APP_Api::PERMISSION_ERROR, "Not Permission.");
        }
    ...
    public function require_administrator()
    {
        $this->_login_required = TRUE;
        $this->_admin_required = TRUE;
    }
